<script>
    ...
    let pythonVariables = $state({});
    let globalVariables = $state({});
    ...
    let unsubscribeFromVariables = null;
    ...
    function getHighlightedCode(code, variables) {
    ...
    onMount(async() => {
        ...
        if (type === 'code') {
            updateImportSuggestions();
            unsubscribeFromVariables = pyodideService.subscribeToVariables((variables) => {
      ...
      <div class="code-cell-content">
        ...
        <div class="code-cell-main">
          <div class="code-editor-container">
            {#if Object.keys(globalVariables).length > 0}
              <div class="code-highlight-layer" aria-hidden="true">
                {@html getHighlightedCode(text, globalVariables)}
              </div>
            {/if}
            <textarea 
              bind:this={textareaElement}
              ...
              oninput={handleInput}
              onblur={stopEditing}
              class="cell-textarea {Object.keys(globalVariables).length > 0 ? 'has-highlighting' : ''}"
            ></textarea>
          </div>
          ...
          <PythonArea 
            ...
            bind:variables={pythonVariables}
          />
        </div>
      </div>
    {/if}
...
<style>
  ...
  .cell-textarea.has-highlighting {
    background: transparent;
    z-index: 2;
    position: relative;
  }

  .code-highlight-layer :global(.defined-variable) {
    background: rgba(0, 149, 242, 0.08);
    color: #0066cc;
    border-radius: 2px;
    padding: 0 1px;
    font-weight: 600;
    border-bottom: 1px solid rgba(0, 149, 242, 0.3);
  }
</style>
